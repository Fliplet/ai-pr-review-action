...
const { MODELS } = __nccwpck_require__(9992);
...
function loadRepoSpecificStandards(repoName) {
  ...
  try {
    const filePath = __nccwpck_require__.ab + "standards/" + fileName;
...
IMPORTANT: Also provide a file_summaries array with a brief 1-line summary (max 80 chars) of what changed in each file. This creates a "walkthrough" section for reviewers. Focus on the semantic change, not just "modified" or "updated".`;
  ...
  try {
    const rulesPath = __nccwpck_require__.ab + "severity-rules.json";
    const rules = JSON.parse(fs.readFileSync(__nccwpck_require__.ab + "severity-rules.json", 'utf-8'));
...
async function reviewWithClaude({ diff, prTitle, prBody, prBase, repoName, fileList, maxOutputTokens, model, enableThinking, complexityScore, fullFileContext, platformImpact }) {
  ...
  const standardsPath = __nccwpck_require__.ab + "fliplet-rules.md";
  const standards = fs.readFileSync(__nccwpck_require__.ab + "fliplet-rules.md", 'utf-8');
...
const { SECURITY_SENSITIVE_PATTERNS, COMPLEXITY_KEYWORDS, MODELS } = __nccwpck_require__(9992);
...
function loadArchitecture() {
  try {
    const archPath = __nccwpck_require__.ab + "fliplet-architecture.json";
    return JSON.parse(fs.readFileSync(__nccwpck_require__.ab + "fliplet-architecture.json", 'utf-8'));
  } catch (err) {
...
const { SECURITY_SENSITIVE_PATTERNS, CORE_FILE_PATTERNS } = __nccwpck_require__(9992);
...
const { parseDiff, isReviewableFile, getTotalChanges } = __nccwpck_require__(160);
const { truncateDiff, formatDiffForPrompt, estimateTokens, isSecurityFile } = __nccwpck_require__(980);
const { reviewWithClaude, triageFiles } = __nccwpck_require__(3267);
const { postReview } = __nccwpck_require__(5770);
const { scorePRComplexity, recommendModel } = __nccwpck_require__(2462);
const { assessPlatformImpact } = __nccwpck_require__(8241);
const { generateTestSuggestions } = __nccwpck_require__(909);
const { suggestLabels, hasTestFiles } = __nccwpck_require__(9215);
const { generateTips } = __nccwpck_require__(1307);
const { detectIssues } = __nccwpck_require__(8085);
const { MODELS, MODEL_PRICING, SECURITY_SENSITIVE_PATTERNS } = __nccwpck_require__(9992);
